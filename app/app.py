...
ALLOWED_EXTENSIONS = {'docx', 'md', 'pdf', 'xlsx', 'csv'}  # Định nghĩa các định dạng file cho phép
...
def start_conversation():
    ...
    try:
        # Tạo thread mới với cơ chế retry
        max_retries = len(ASSISTANT_IDS)
        current_retry = 0
        thread_id = None
        ...
        # Tiếp tục với phần code hiện tại để tạo conversation
        url = f'{CHATBOT_URL}/chat-messages'
        headers = {
            'Authorization': f'Bearer {CHATBOT_APIKEY}',
            'Content-Type': 'application/json'
